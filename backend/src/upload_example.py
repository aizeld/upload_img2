...
BASE_URL = 'https://dev1.agroonline.kzi/api'  # FOR EU
...
def get_fields(access_token, company_id):
    # URL для запроса информации после аутентификации
    data_url = f"{BASE_URL}/company/{company_id}/fields/"

    # Заголовки для запроса данных, включая токен аутентификации
    ...
    if data_response.status_code == 200:
        ...
        raise Exception(f"Error: {data_response.status_code}")
...
def test():
    LOGIN = 'XXXXXX'
    PASSWORD = 'XXXXXXXX'
    COMPANY_ID = 807  # ID компании

    token = get_token(LOGIN, PASSWORD)
    # test fields
    fields = get_fields(token, COMPANY_ID)
    # test upload image to server
    PATH_TO_IMG_FILE = '/home/mxl/Documents/14072023_59158r2_NDVI_P20160317_S2A.png'
    # ревизий больше нет, есть только текущая форма поля
    upload_image(token, COMPANY_ID, get_test_data_for_image_upload(), PATH_TO_IMG_FILE)
